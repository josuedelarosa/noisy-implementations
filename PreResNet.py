...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        ...
        # Key change #1: allow grayscale by passing in_channels=1
        ...
        # Key change #2: adaptive pooling so it works for 64x64, 128x128, etc.
...
def ResNet18(num_classes=10, in_channels=1):
    return ResNet(PreActBlock, [2, 2, 2, 2], num_classes=num_classes, in_channels=in_channels)
...
def test():
    # CIFAR-style RGB
    net_rgb = ResNet18(num_classes=10, in_channels=3)
    y = net_rgb(Variable(torch.randn(2, 3, 32, 32)))
    print("RGB out:", y.size())

    # LIDC grayscale example
    net_gray = ResNet18(num_classes=2, in_channels=1)
    y2 = net_gray(Variable(torch.randn(2, 1, 128, 128)))
    print("Gray out:", y2.size())
